...
public class MetricaSaludTools {
	...
	private final MetricaSaludServiceImpl metricaSaludService;
	
	
	@Tool(description = "Get the total sleep evolution for the user in this week. If there are missing entries, ignore them.")
	public String getTimeSleepByWeek() throws NotFoundException {
		UUID idUsuario = UUID.fromString("b2f22e50-321d-4816-a8bd-7a0670b72045");
		LocalDate today = LocalDate.now();
		LocalDate firstDayOfWeek = today.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
		LocalDate lastDayOfWeek = today.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY));
		
		List<MetricaSalud> registros =  metricaSaludService.getRegistrosDiariosByUserInRange(idUsuario, firstDayOfWeek, lastDayOfWeek).orElseThrow(NotFoundException::new);
